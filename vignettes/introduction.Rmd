...
Another way to assess wide effect modification is to compute the standardized total gain (STG) [@Gabriel15]. This is implemented in the `calc_STG` function. The standardized total gain can be interpreted as the area sandwiched between the risk difference curve and the horizontal line at the marginal risk difference. It is a measure of the spread of the distribution of the risk difference, and is a less parametric way to test for wide effect modification. The `calc_STG` function computes the STG at the estimated parameters, at the bootstrap samples, if present and provides a 95% percentile CI. It should be noted that as pointed out in @Gabriel14, this CI will almost never include 0, as STG is always positive and therefore is not of great use for the evaluation of surrogates having any value. The CI instead can be used to compare surrogates. *In a future update we will include a p-value and bootstrap for the comparison of STGs between surrogates.* The function prints the results and invisibly returns a list containing the observed STG and the bootstrapped STGs. If the outcome is time to event the STG is time-dependent, and a time point for evaluation is needed, if one is not provided then the restricted mean survival is estimated from the data and used. 
