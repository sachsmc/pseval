...
```{r logrr}
plot(binary.boot, summary = "logRR", lwd = 2)
```

The `VE` function is the workhorse that creates the plots. You can call this function directly to obtain estimates, standard errors, and confidence intervals for the estimated risk in each treatment arm and transformations of the risk like VE. The parameter `n.samps` determines the number of points at which to calculate the VE. The points are evenly spaced over the range of S.1. Use this function to compute other summaries, make plots using `ggplot2` or `lattice` and more. 

```{r ve}
ve.est <- VE(binary.boot, CI.type = "pointwise", n.samps = 200)
head(ve.est)
```
...
```{r plotciag}
plot(binary.boot, summary = "VE", lwd = 2, CI.type = "band")
sbs <- VE(binary.boot, CI.type = "pointwise", n.samps = 200)
lines(VE.lower.CL.2.5 ~ S.1, data = sbs, lty = 3, lwd = 2)
lines(VE.upper.CL.97.5 ~ S.1, data = sbs, lty = 3, lwd = 2)
...
```{r ggpt}
library(ggplot2)
VE.est <- VE(binary.boot, n.samps = 200)
ggplot(VE.est, 
       aes(x = S.1, y = VE, ymin = VE.lower.CL.0.95, ymax = VE.upper.CL.0.95)) + 
