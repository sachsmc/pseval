...
This procedure is called estimated maximum likelihood (EML) and was developed in @Pepe91. The key idea is that we are averaging the likelihood contributions for subjects missing $S(1)$ with respect to the estimated distribution of $S(1) | W$. A BIP $W$ that is strongly associated with $S(1)$ is needed for adequate performance. Closed-form inference is not available for EML estimates, thus we recommend use of the bootstrap for estimation of standard errors. It was suggested as an approach to prinicipal surrogate evaluation by @Gilbert08 and @Huang11.


##Psuedoscore 

@Huang12 suggest a different estimation procedure that does have a closed form variance estimates are known. Psuedscore estimates were suggest in @WolfsonDiss and implemented for several special cases in @Huang12. 
